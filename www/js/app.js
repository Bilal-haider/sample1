	...
	success: function (data) {	  
		...
		if (data.code==1){
			switch (action)
			{
				...
				var merch = getStorage('merchant_id');
					var serv=localStorage.getItem(merch+"service");
	var delpr=localStorage.getItem(merch+"delss");
    var minor=localStorage.getItem(merch+"minor");   
								
				if(delpr.trim().length=='0') {
					delpr='0';
					
				}
				
				$('.myservicess').html(serv);
				$('.delprr').html(delpr);
				$('.minorprr').html(minor);
			   ...
			   case "applyVoucher":   
			    ...
			    case "GetCategoryList":
			       $("#page-menubycategoryitem #search-text1").html(data.details.category_title);
...
{	
    ...
    $.each( data, function( key, val ) {     
    	 ...
    	 //dump(val);
		  ...
		  localStorage.setItem(val.merchant_id+"service",'');
		  localStorage.setItem(val.merchant_id+"delss",'0');
    	  localStorage.setItem(val.merchant_id+"minor",'0');
		   localStorage.setItem(val.merchant_id+"openss",''); 		
		 
    	 htm+='<ons-list-item modifier="tappable" class="list-item-container listresto" onclick="loadRestaurantCategory('+val.merchant_id+');" >';
    	 //htm+='<ons-row class="row" onclick="loadRestaurantCategory('+val.merchant_id+');" >';    	 
    	 htm+='<ons-row class="row retrow" >';    	 
    	     ...
    	     htm+='<ons-col class="col-description border" width="65%">';
    	           htm+='<div>';
	    	          ...
	    	          htm33='<span class="notification '+val.tag_raw+' ">'+val.is_open+'</span>';
					  
					   localStorage.setItem(val.merchant_id+"openss",htm33);
    	           htm+='</div>';
    	           ...
    	           htm+='<ons-row>';
    	                  ...
    	                  // htm+='<p class="p-small trn" data-trn-key="delivery">Delivery</p>';
    	                   if(!empty(val.delivery_fee)){
    	                     // htm+='<price>'+val.delivery_fee+'</price>';
							  localStorage.setItem(val.merchant_id+"delss",val.delivery_fee);
    	                   }
    	                 }
    	              htm+='</ons-col>';
    	             ...
    	             // htm+='</ons-col>';
					  localStorage.setItem(val.merchant_id+"minor",val.minimum_order);
    	              }
			 ...
			 htm+='<ons-col class="col-image border" width="35%">';
    	          htm+='<div class="logo-wrap2" >';
    	            htm+='<div class="img_loaded" >';
    	             htm+='<img class="alignment" src="'+val.logo+'" />';
    	            htm+='</div>';
    	         ...
    	         htm2='';
    	          ...
    	          if(!empty(val.services)){    	          	
    	          	    $.each( val.services, function( key_service, val_services ) { 
						   ...
						   htm2+='<p class="margg title concat-text w5"><i class="green-color1 ion-android-checkmark-circle merg"></i>'+val_services+' </p>';
						  
						   localStorage.setItem(val.merchant_id+"service",htm2);
...
{			
   var merch = getStorage('merchant_id');
	var openss = localStorage.getItem(merch+"openss");
	$('.openorclose').html(openss);
...
function showMerchantInfo(data)
{
	...
	if (!empty(data.payment_method)){
		var p='';
		p+='<ons-list-header class="center trn backsg white pads bold fa fa-dollar " data-trn-key="payment_methods">Payment Methods</ons-list-header>';
